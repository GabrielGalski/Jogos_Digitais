extends Area2D
class_name MimicElite

const MIMIC_PROJECTILE_SCENE := preload("res://scenes/enemies/projectiles/mimic_projectile.tscn")
const FLEE_DIRECTION_SAMPLES := 16

@export var movement_speed := 31.0
@export var max_resistance := 12
@export var burst_interval := 1.5
@export var burst_size := 3
@export var burst_shot_interval := 0.12
...
func _physics_process(delta: float) -> void:
	...
	_update_flee_movement(delta)
	_update_burst(delta)


func _update_flee_movement(delta: float) -> void:
...
func _update_burst(delta: float) -> void:
	if burst_shots_remaining == 0:
		burst_elapsed += delta
		if burst_elapsed >= burst_interval:
			burst_elapsed = fmod(burst_elapsed, burst_interval)
			burst_shots_remaining = burst_size
			burst_shot_elapsed = 0.0

	if burst_shots_remaining <= 0:
		return
	burst_shot_elapsed -= delta
	if burst_shot_elapsed <= 0.0:
		_fire_projectile()
		burst_shots_remaining -= 1
		burst_shot_elapsed = burst_shot_interval
...
func _fire_projectile() -> void:
	...
	var projectile := MIMIC_PROJECTILE_SCENE.instantiate()
	get_parent().add_child(projectile)
	projectile.global_position = global_position + shot_direction * (separation_radius + 4.0)
	projectile.call(&"setup", target, shot_direction)
